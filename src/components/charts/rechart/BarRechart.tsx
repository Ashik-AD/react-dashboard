...
const BarRechart = () => {
    const {data, loading} = useFetch<any[]>('/api/chart-data/rechart/turnover');
    if(!data || loading) return <Box height='400px'>{""}</Box>
  return (
    <ResponsiveContainer height={400} width="100%">
        <BarChart data={data} barCategoryGap="10%" barSize={15} barGap={4} margin={{left: -20}}>
            <XAxis dataKey={"name"} />
            <YAxis />
            <Tooltip content={<CustomTooltip />} cursor={{opacity: 0.1}} />
            <Legend verticalAlign="top" height={50} content={<CustomLegend style={{marginLeft: 24}} />} />
            ...
            <Bar dataKey={"sony"} stackId="a" fill='#dba65c' width={30} radius={[15, 15, 0, 0]} />
        </BarChart>
    </ResponsiveContainer>
  )
}
